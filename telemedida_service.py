...
class TelemedidaService:
    ...
    def load_config(self):
        ...
        # Fecha de filtro (usa fecha actual por defecto)
        filter_date_str = os.getenv('FILTER_DATE', datetime.now().strftime('%Y-%m-%d'))
        self.fecha_filtro = datetime.strptime(filter_date_str, '%Y-%m-%d').replace(tzinfo=timezone.utc)
    ...
    def get_google_sheets_data(self):
        ...
        self.logger.info(f"Data OK filas de BD_Telemedida = {len(df_sheet)}")
        return df_sheet
